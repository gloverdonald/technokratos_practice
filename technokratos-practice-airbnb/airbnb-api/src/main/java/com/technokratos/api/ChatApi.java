...
public interface ChatApi<PRINCIPAL> {
    ...
    MessageResponse createMessage(@ApiParam(value = "Сообщение") @Valid @RequestBody MessageRequest message);
    ...
    MessageResponse updateMessage(@ApiParam(value = "Идентификатор сообщения") @PathVariable("id") UUID id,
                                  @ApiParam(value = "Обновленное сообщение") @Valid @RequestBody MessageRequest message,
                                  @AuthenticationPrincipal PRINCIPAL userPrincipal);
